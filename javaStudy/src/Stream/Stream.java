...
//      1-1. 배열 스트림
        ...
        java.util.stream.Stream stream1 = Arrays.stream(arr);
...
//      1-3. 스트림 builder
        ...
        java.util.stream.Stream streamBuild = java.util.stream.Stream.builder().add("a").add("b").add("c").build();
        streamBuild.forEach(System.out::println);


//      1-4. 스트림 generate
        System.out.println("== 스트림 generate ==");
        java.util.stream.Stream streamGenerate = java.util.stream.Stream.generate( () -> "abc" ).limit(3);
...
//      3-3. reduce
        ...
        java.util.stream.Stream<Integer> stream3 = new ArrayList(Arrays.asList(1, 2, 3, 4, 5)).stream();
